...
# Select all
planet_db = pd.read_sql("""SELECT * FROM planets; """, conn1)
print(planet_db)
...
df_mass_moon = pd.read_sql("""
    SELECT * FROM planets
    WHERE mass < 1.00 AND num_of_moons >= 1; """, conn1)
print(df_mass_moon)
...
# Select all
dog_db =pd.read_sql("SELECT * FROM dogs;", conn2)
print(dog_db)
...
df_hungry_ages = pd.read_sql("""
    ...
    FROM dogs
    WHERE hungry = 1 AND age > 2 AND age < 8
...
df_ruth_years = pd.read_sql("""
    SELECT max(year) - min(year) + 1 AS total_years_played 
